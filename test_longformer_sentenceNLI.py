...
def load_MNLI():
    ...
    for row in readfile:
        if line_co>0:
            ...
            text_a = line[8].strip()
            text_b = line[9].strip()
            label = 'entailment' if line[-1].strip() == 'entailment' else 'not_entailment'# if  #["entailment", "neutral", "contradiction"]
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
            label_in_3way.append(line[-1].strip())
        line_co+=1
...
def main():
    ...
    # )
    parser.add_argument(
        "--per_gpu_train_batch_size", default=10, type=int, help="Batch size per GPU/CPU for training.",
    )
    parser.add_argument(
    ...
    longformer_path = '/export/home/Dataset/BERT_pretrained_mine/paragraph_entail/longformer_full_pair/roberta_f1.dev.0.8643652333775916.test0.8794932985126149'
    # longformer_path = '/export/home/Dataset/BERT_pretrained_mine/paragraph_entail/longformer_full_pair_largestTrain/roberta-f1.dev.0.8436753079610222.test0.8580708661417322'
    # longformer_path = '/export/home/Dataset/BERT_pretrained_mine/paragraph_entail/longformer_full_pair_rawPlusFine/roberta-epoch5'
